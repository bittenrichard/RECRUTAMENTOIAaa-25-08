...
app.post('/api/jobs', async (req, res) => {
    ...
    const { titulo, descricao, endereco, modo_trabalho, requisitos_json, requisitos_obrigatorios, requisitos_desejaveis, usuario } = req.body;
...
app.patch('/api/jobs/:jobId', async (req, res) => {
    ...
    const { titulo, descricao, endereco, modo_trabalho, requisitos_json, requisitos_obrigatorios, requisitos_desejaveis } = req.body;
...
app.patch('/api/candidates/:candidateId', async (req, res) => {
    const { candidateId } = req.params;
...
app.delete('/api/candidates/:candidateId', async (req, res) => {
    const { candidateId } = req.params;
    try {
...
app.get('/api/public/theoretical-models', async (req, res) => {
    try {
        ...
        const filteredResults = response.results.filter((model) => {
...
app.get('/api/public/theoretical-test/:testId', async (req, res) => {
    const { testId } = req.params;
    if (!testId) {
...
app.patch('/api/theoretical-test/submit', async (req, res) => {
...
app.delete('/api/theoretical-test/cancel/:testId', async (req, res) => {
    const { testId } = req.params;
    if (!testId) {
        return res.status(400).json({ error: 'ID da prova é obrigatório.' });
    }
...
app.get('/api/theoretical-test/check/:candidateId', async (req, res) => {
    const { candidateId } = req.params;
    // Pegar o ID do usuário dos headers
    const userId = req.headers['x-user-id'] || req.query.userId || '1';
    if (!candidateId) {
...
app.post('/api/theoretical-templates/:templateId/duplicate', async (req, res) => {
    try {
...
app.post('/api/theoretical-test/generate', async (req, res) => {
    ...
    const { candidateId, modeloId, recruiterId, candidato_id, modelo_prova_id } = req.body;
...
app.delete('/api/theoretical-test/delete/:testId', async (req, res) => {
    const { testId } = req.params;
    if (!testId) {
        return res.status(400).json({ error: 'ID da prova é obrigatório.' });
    }
...
app.get('/api/theoretical-test-results/:candidateId', async (req, res) => {
    try {
        ...
        const testsWithModels = await Promise.all(response.results.map(async (test) => {
            try {
