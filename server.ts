...
class AutoMatchService {
   ...
   */
  async analyzeJobCandidateMatch(job: any, candidate: any): Promise<{
    score: number;
    reasons: string[];
...
Retorne SEMPRE um JSON válido com formato: {"score": número, "resumo": "texto"}`
   ...
   */
  buildIntelligentPrompt(job: any, candidate: any): string {
    // Extrair requisitos da vaga (requisitos_json - criado pela campanha)
    let requisitosVaga: any = {};
    try {
 ...
 */
app.post('/api/auto-match/test-one', async (req: Request, res: Response) => {
...
app.post('/api/auto-match/execute', async (req: Request, res: Response) => {
  ...
  try {
    ...
    while (hasMore) {
      const candidatesResult = await baserowServer.get(
        CANDIDATOS_TABLE_ID, 
        `?filter__usuario__link_row_has=${userId}&size=200&page=${page}`
      );
    ...
    const analysisPromises = candidatesParaAnalisar.map(async (candidate, index) => {
      try {
        ...
        if (!candidate.cv_completo || !candidate.cv_completo.trim()) {
          ...
          await autoMatchService.updateCandidateScore(
            candidate.id,
            jobId,
            0,
            'Candidato sem CV completo no sistema',
            { nota: 0, justificativa: 'CV não disponível' }
          );
        ...
        await autoMatchService.updateCandidateScore(
          candidate.id,
          jobId, // Passar ID da vaga para vincular se score ≥60%
          analysis.score,
 ...
 */
app.get('/api/auto-match/results/:jobId', async (req: Request, res: Response) => {
  const { jobId } = req.params;
  
  try {
    const resultsData = await baserowServer.get(
      AUTO_MATCH_RESULTS_TABLE_ID, 
      `?filter__job_id__equal=${jobId}`
    );
  ...
  try {
    ...
    const syncedCandidates = userCandidatesRaw.map((candidate: BaserowCandidate) => {
      ...
      const enrichedCandidate = {
        ...
        theoretical_tests: theoreticalTests.map((test: any) => ({
          id: test.id,
...
app.post('/api/upload', upload.any(), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [jobInfo, userInfo] = await Promise.all([
      getCachedData(`job_${jobId}`, () => baserowServer.getRow(VAGAS_TABLE_ID, parseInt(jobId as string))),
      getCachedData(`user_${userId}`, () => baserowServer.getRow(USERS_TABLE_ID, parseInt(userId as string)))
    ]);
